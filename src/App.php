...
final class App
{
    ...
    private function home(): void
    {
        ...
        $now = new \DateTimeImmutable('now', new \DateTimeZone(Config::get('APP_TIMEZONE', 'America/Sao_Paulo')));
        $isOpen = (int)$now->format('N') !== 1 && $now->format('H:i') >= '18:00' && $now->format('H:i') <= '23:30';
        $eta = $this->db->query('SELECT MIN(eta_minutes) mn, MAX(eta_minutes) mx FROM delivery_areas WHERE active=1')->fetch();
        $etaLabel = ($eta && $eta['mn']) ? ((int)$eta['mn'] === (int)$eta['mx'] ? (int)$eta['mn'].' min' : (int)$eta['mn'].'–'.(int)$eta['mx'].' min') : null;
        ob_start(); ?>
        <section class="hero"><img src="/assets/hero.jpg" alt="Hambúrguer artesanal Burguer App"><div><span>CARNE, FOGO & PERSONALIDADE</span><h1>A NOITE PEDE<br><em>BURGUER.</em></h1><p>Hambúrguer artesanal de verdade, criado na brasa e entregue quente até você.</p><p class="status-badge <?=$isOpen?'is-open':'is-closed'?>"><span class="dot"></span><?=$isOpen?'Aberto agora':'Fechado agora'?><?php if($etaLabel):?><span class="sep">·</span><?=Security::e($etaLabel)?><?php endif;?></p><a class="primary" href="#cardapio">Pedir agora</a></div></section>
        ...
        <?php foreach ($groups as $category => $items): ?><div class="product-row" id="<?=Security::e($this->slug($category))?>"><header><h3><?=Security::e(mb_strtoupper($category))?></h3><span class="row-arrows" aria-hidden="true">←&nbsp;&nbsp;→</span></header><div class="products">
    ...
    private function admin(): void
    {
        ...
        ob_start();?><div class="admin-shell"><aside id="admin-sidebar"><img src="/assets/logo-burguer-app.jpg"><b>APP</b><span>PAINEL DE GESTÃO</span><nav><?php foreach(['overview'=>'Visão geral','orders'=>'Pedidos','products'=>'Produtos','categories'=>'Categorias','addons'=>'Adicionais','coupons'=>'Cupons','areas'=>'Áreas de entrega','banners'=>'Banners e promoções','settings'=>'Configurações','users'=>'Usuários']as$k=>$v):?><a class="<?=$page===$k?'active':''?>" href="/admin?page=<?=$k?>"><?=Security::e($v)?></a><?php endforeach;?></nav><a href="/">← Ver loja</a></aside><main><header><div class="admin-header-title"><button type="button" class="sidebar-toggle" aria-expanded="false" aria-controls="admin-sidebar" aria-label="Abrir menu"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg></button><div><span class="eyebrow">BURGUER APP</span><h1><?=Security::e(mb_strtoupper($page))?></h1></div></div><form method="post" action="/logout"><input type="hidden" name="_csrf" value="<?=Security::e(Security::csrf())?>"><button>Sair</button></form></header>
    ...
    private function layout(string $title,string $content,bool $admin=false):void{$flash=$_SESSION['flash']??null;unset($_SESSION['flash']);$cartCount=0;foreach(($_SESSION['cart']??[])as$line)$cartCount+=is_array($line)?(int)($line['quantity']??0):(int)$line;$whatsapp=preg_replace('/\D/','',Config::get('WHATSAPP_NUMBER',''));?><!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?=Security::e($title)?></title><link rel="stylesheet" href="/assets/app.css"></head><body class="<?=$admin?'admin-body':''?>"><?php if(!$admin):?><header class="topbar"><a href="/" class="brand"><img src="/assets/logo-burguer-app.jpg"><b>BURGUER APP</b></a><nav id="site-nav"><a href="/">Início</a><a href="/#cardapio">Cardápio</a><a href="/#promocoes">Promoções</a><a href="/#duvidas">Dúvidas</a></nav><div class="topbar-actions"><a class="cart-link" href="/checkout" aria-label="Abrir sacola"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.5 8.5h11l1.2 11H5.3l1.2-11Z"/><path d="M9 9V6a3 3 0 0 1 6 0v3"/></svg><span>Sacola (<?=$cartCount?>)</span></a><?php if(Auth::check()):?><form method="post" action="/logout"><input type="hidden" name="_csrf" value="<?=Security::e(Security::csrf())?>"><button>Sair</button></form><?php else:?><a class="login-link" href="/login" aria-label="Entrar"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z"/><path d="M4.5 20a7.5 7.5 0 0 1 15 0"/></svg></a><?php endif;?><button type="button" class="nav-toggle" aria-expanded="false" aria-controls="site-nav" aria-label="Abrir menu"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M4 12h16M4 17h16"/></svg></button></div></header><?php endif;?><?php if($flash):?><div class="toast <?=Security::e($flash['type'])?>"><?=Security::e($flash['message'])?></div><?php endif;?><?=$content?><?php if(!$admin && $whatsapp!==''):?><a class="whatsapp" href="https://wa.me/<?=Security::e($whatsapp)?>" rel="noopener noreferrer" target="_blank"><img src="/assets/whatsapp.svg" alt=""><span>Fale com a Burguer</span></a><?php endif;?><script src="/assets/app.js" defer></script></body></html><?php }
